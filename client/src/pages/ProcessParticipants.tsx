                  ...
                  {participants.map((participant: any) => {
                    ...
                    return <div key={participant.id} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
                      <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-3 mb-3">
                        ...
                        <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setExpandedParticipantId(isExpanded ? null : participant.id)} className="gap-1"><UsersRound size={14} />Trabajadores y KPI {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</Button><Button size="sm" variant="outline" onClick={() => handleEdit(participant)} className="gap-1"><Edit2 size={14} />Editar</Button><Button size="sm" variant="destructive" onClick={() => confirm("¿Estás seguro de que deseas eliminar este participante?") && deleteMutation.mutate({ id: participant.id })} className="gap-1"><Trash2 size={14} />Eliminar</Button></div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div><p className="font-semibold text-slate-700">Objetivo:</p><p className="text-slate-600 whitespace-pre-wrap">{participant.objective || "—"}</p></div>
                        <div><p className="font-semibold text-slate-700">Responsabilidades:</p><p className="text-slate-600 whitespace-pre-wrap">{participant.responsibility || "—"}</p></div>
                        <div><p className="font-semibold text-slate-700">Autoridad:</p><p className="text-slate-600 whitespace-pre-wrap">{participant.authority || "—"}</p></div>
                      </div>
