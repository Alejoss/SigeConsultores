...
export default function ManagementPrograms() {
  ...
  const overallCompliance = useMemo(() => {
    ...
    const total = programs.reduce((sum: number, p: Program) => {
      const pct = p.plannedActions > 0
        ? Math.min(100, Math.round((p.completedActions / p.plannedActions) * 100))
        : 0;
      return sum + pct;
    }, 0);
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {(programs as Program[]).map((program) => {
              const compliance = program.plannedActions > 0
                ? Math.min(100, Math.round((program.completedActions / program.plannedActions) * 100))
                : 0;
              return (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                      ...
                      <div>
                        ...
                        <Input
                          type="number"
                          min={0}
                          defaultValue={program.plannedActions}
                      ...
                      <div>
                        ...
                        <Input
                          type="number"
                          min={0}
                          defaultValue={program.completedActions}
